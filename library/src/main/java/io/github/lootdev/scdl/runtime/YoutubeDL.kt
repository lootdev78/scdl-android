...
import io.github.lootdev.scdl.runtime.mapper.VideoInfo
import io.github.lootdev.scdl.common.SharedPrefsHelper
import io.github.lootdev.scdl.common.SharedPrefsHelper.update
import io.github.lootdev.scdl.common.utils.ZipUtils.unzip
...
object YoutubeDL {
    ...
    /** Returns the embedded Python executable after [init] has completed. */
    fun pythonExecutable(): File {
        assertInit()
        return pythonPath!!
    }

    /** Returns the bundled yt-dlp zip application after [init] has completed. */
    fun ytdlpExecutable(): File {
        assertInit()
        return ytdlpPath!!
    }

    /** Returns the bundled FFmpeg executable after [init] has completed. */
    fun ffmpegExecutable(): File {
        assertInit()
        return ffmpegPath!!
    }

    /** Returns the bundled QuickJS executable after [init] has completed. */
    fun quickJsExecutable(): File {
        assertInit()
        return quickJsPath!!
    }

    /**
     * Runs an importable module inside the embedded Python runtime.
     *
     * This is intentionally generic so the SCDL core module can reuse the
     * Python runtime without duplicating native binaries. Python packages can be supplied through
     * [environmentOverrides], most commonly by setting PYTHONPATH.
     */
    @JvmOverloads
    @Throws(YoutubeDLException::class, InterruptedException::class, CanceledException::class)
    fun executePythonModule(
        moduleName: String,
        ...
        command.addAll(listOf(pythonPath!!.absolutePath, "-m", moduleName))
        command.addAll(arguments)

        val processBuilder = ProcessBuilder(command)
            .redirectErrorStream(redirectErrorStream)
        if (workingDirectory != null) {
            require(workingDirectory.isDirectory) { "workingDirectory must be an existing directory" }
            processBuilder.directory(workingDirectory)
        }
        applyPythonEnvironment(processBuilder, environmentOverrides)
        ...
        if (exitCode > 0) {
            ...
            val message = err.ifBlank { out }.ifBlank { "Python module exited with code $exitCode" }
            throw YoutubeDLException(message)
        }
        idProcessMap.remove(processId)
    ...
    private fun applyPythonEnvironment(
    ...
    private fun executeImpl(
        ...
        applyPythonEnvironment(processBuilder)
    ...
    const val baseName = "scdl-runtime"
